...
def main():
    with open('assets/first_cosine_values.txt', 'r', encoding='utf8') as source:
        ...
        score_contents = score_contents[:30]
        score_contents = [content['word'] + '=' + content['score'] for content in score_contents]

    with open('assets/first_cosine_values_ranked.txt', 'w', encoding='utf8') as result:
        result.write('\n'.join(score_contents))
